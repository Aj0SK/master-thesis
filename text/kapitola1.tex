...
In the next part, we describe how we search for some arbitrary pattern $P$
...
{\tt house} as a prefix we look into the range of all prefixes beginning with {\tt h}. Locating this
...
starting with {\tt house} will be located, but we do not store anything except for $T_{BWT}$ and $\Count$.
...
In general, we first find the subsequence of $M$s rows, where suffix $P_{n-1\ldots}$ is located.
...
that has as a prefix $P_{n-2\ldots}$. As we already located rows beginning with $P_{n-1\ldots}$, we can reuse
