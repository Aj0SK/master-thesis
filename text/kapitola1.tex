...
In general, we first find the subsequence of $M$s row,
where $P[n-1\ldots]$ is located. As this is just a one symbol $P[n-1]$ the initial subsequence is
just a run of symbol $P[n-1]$ in $F$ given by
\begin{align*}
	b_{n-1}&=\Count[P[n]] \\
	e_{n-1}&=\Count[P[n]+1].\\
\end{align*}
to find where $P[n-1\ldots]$ is located in $M$, we would like to locate occurrences of $P[n]$ that
are preceded by $P[n-1]$. We thus look at the run of symbol $P[n-1]$ in $F$ that spans from $\Count[P[n-1]]$
up to $\Count[P[n-1]+1]$. To find the sequence of rows starting with $P[n-2\ldots ]$, given by $b_{n-2}$
and $e_{n-2}$, we need to find where it beggins and ends in the run of character $P[n-2]$ in $F$. Thanks
to property \ref{chapter1:fmindexprop:prop2}, we just need to find the number of occurences of character
$P[n-2]$ in $L$ up to the $b_{n-1}$, the start of previous subsequence of rows that contain $P[n-1\ldots ]$ as
prefix and also the number of occurences of character $P[n-2]$ in $L$ up to the $e_{n-1}$ giving us 
\begin{align*}
