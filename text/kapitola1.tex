...
$\mathit{rank}_1(i)$. If we are provided with bit vector implementation along with access and
...
answer to this query -- the position of $i$-th one -- $\mathit{select}_1(i)$. In
...
\begin{itemize}
	...
	$b_{n-2} = mathit{Count}[P[n-2]] + mathit{rank}_{P[n-2]}(b_{n-1})$ and
	$e_{n-2} = mathit{Count}[P[n-2]] + mathit{rank}{P[n-2]}(e_{n-1})$. Here $mathit{Count}[P[n-2]]$
