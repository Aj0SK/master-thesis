...
% TODO: mozno vymazat pouzivanie "character" a radsej vsade "symbol"
...
reduces the usability of data structure and often completely ruins runtime due to
...
\begin{itemize}
	\item \textit{implicit} - if it takes $I + \BigO(1)$ bits of space,
	\item \textit{succinct} - if it takes $I + o(I)$ bits of space and
	\item \textit{compact} - if it takes $\BigO(I)$ bits of space.
\end{itemize}
...
and $1\leq k\leq \#_c(S)$ where $\#_c(S)$ denotes the number of occurences of symbol $c$ in $S$. The first
...
Let us now consider practically interesting application within a more complex data structure.
As we shall see, data structure used to solve this problem also uses bit vector and other
...
sorted according to the lexicographical order. We shall use the helper sequence $\Count$ where
$\Count[c]$ is number of occurrences of symbols smaller than $c$. Note that the run of symbol $c$ in
$F$ starts at the index $\Count[c]$.
...
To search for the range of rows in $M$ that have {\tt house} as a prefix we look into the range of all
...
range where suffixes starting with {\tt ouse} are located in. We can count how many times {\tt h} is located
...
% TODO: pridat obrazok, skontrolovat indexovanie

In general, to count number of occurences of pattern $P$, we first find the subsequence of rows of $M$, where
suffix $P_{n-1\ldots}$ is located. As this is just a one symbol, $p_{n-1}$, the initial subsequence is a run
...
compute the number of occurences of character $p_{n-2}$ in $L$ up to the $b_{n-1}$, the start of previous
subsequence of rows and also the number of occurences of character $p_{n-2}$ in $L$ up to the $e_{n-1}$
...
We may observe that in FM-index, we use $\rank$ on sequence over general aphabet. This application
...
reasonable implementation of the $\rank$/$\select$ methods on the sequence over general alphabet.
...
straightforward approach using bit vector is to have one bit vector $B_c$ for every symbol $c$
...
concatenation of bits that are contained in edges along the path from root to the leaf node
containing symbol $c$.

The idea proposed by~\cite{makinen2005succinct} is to shape the wavelet tree in such a way that
a code of every symbol is equal to Huffman code of this symbol. The advantage we get is that
if queries for a symbols are proportional to their frequencies, then we need to visit on average
...
where $\#_c(S)$ denotes number of occurences of symbol $c$ in $S$. Even if the maximum depth of this
Huffman shaped wavelet tree may be bigger compared to the original, \cite{grabowski2004first} showed
that we can enforce the maximum depth to be $\BigO(\log \sigma)$ with the average depth limited by
$H_0+2$. This is, however, as they claimed, not very practical. The most important fact is that
with Huffman shaped wavelet tree, it is possible to decrease the average number of nodes accessed and
thus decrease the average query time of $\rank$ and $\select$.
