...
a symbols are proportional to their frequencies, then we need to visit on average
only $\BigO(H_0)$ nodes rather than $\BigO(\log \sigma)$, where $H_0$ is empirical zero-order
entropy, defined as $$H_0(S)=\sum_{c\in\Sigma} \frac{n_c}{n} \log \frac{n}{n_c}$$,
where $n_c$ denotes number of occurences of symbol $c$ in sequence $S$ of length $n$. The value
