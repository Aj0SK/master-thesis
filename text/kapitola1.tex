...
close to $nH_k(T)$ for the text $T$ of length $n$ over alphabet of size $\sigma$ where $H_k$ is
...
symbol of $S$. The example of the data structure obtaining bound $nH_k(T)+\BigO((n\log\log n)/\log_{\sigma}n)$
is AF-index (alphabet friendly FM-index) proposed by \cite{ferragina2004alphabet}.
