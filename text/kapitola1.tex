...
FM-index stores columns $F$ and $L$. As the first column of $M$ consists of runs of sorted symbols,
it is represented in $FM-index$ using the helper array $\Count$ where $\Count[c]$ is the number of
occurrences of symbols \emph{preceding} $c$. Note that the run of symbol $c$ in $F$ starts at the
index $\Count[c]$.
...
\begin{figure}
	...
	$\Count{[}h{]}$ & 100        & h &  & a & s & h & t &   &   &   &   \\ \cline{5-10}\cline{2-3}\cline{12-12}
	...
	$b_1$           & 200        & o &  & u & s & e &   &   &   &   & m \\ \cline{5-10}\cline{2-3}\cline{12-12}
				    ...
				    & 203        & o &  & u & s & e &   &   & s &   & p \\ \cline{5-10}\cline{2-3}\cline{12-12}
...
a leaf containing queried symbol $c$. Regarding space usage of wavelet tree, it is possible
for this representation to use just $n\lg \sigma+o(n)$ bits of space where $n$ is the length
of represented text $S$ as on every level of the wavelet tree, we store roughly a bit vector
of length $n$.

Even if this version of wavelet tree can be used inside of the FM-index, it is possible to
make a solution that is faster and more space efficient in some scenarios. Speed of both
methods, $\rank_c$ and $\select_c$, depends on the speed of the bit vector implementation,
used in the nodes, but also on the depth of symbol $c$ in the tree. This depth is now for
every single symbol roughly $\BigO(\log \sigma)$.
...
It is also possible for the original shape of the wavelet tree to achieve space usage of $nH_0(S) + o(n\log\sigma)$
bits. This was shown by \cite{grossi2003high} and this representation relies on using compressed
bit vector implementation, that takes space close to the zero-order entropy, inside of the tree's nodes.

\paragraph{Space usage of FM-index}

We already showed how FM-index is constructed, how the searching in FM-index works and how it can be
supported should we have a data structure that supports $\rank$ and $\select$ on sequence over the general
