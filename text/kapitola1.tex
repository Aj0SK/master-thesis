...
In the next sections, we introduce some interesting and useful applications of the bit vector.
...
we shall store alongside this a helper array $P$. Number on $i$-th position is the index where the $i$-th
...
is reasonable and may be used is constructing \textit{suffix array} of $T$. Suffix array of $T$
is data structure that stores information about the lexicograpical order of suffixes of $T$.
...
as their prefix. If we look to the last column, these symbols corresponds to the symbols preceding
{\tt ouse} in the text. We can maybe see there in arbitrary order symbols like {\tt m} for worw
...
second character so there will be some lines continuing with {\tt a} if the text contained for exampl
