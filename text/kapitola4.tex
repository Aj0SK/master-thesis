...
these block sizes, \texttt{SDSL} implements 128 and 256 bit integers. \texttt{SDSL} provides template
...
There are varios reasons why we focus on this part of algorithm. The first one is that for smaller blocks 
...
solved in parallel (by instruction-level parallelism), this part is harder to parallelize. Dividing
the problem into sub-problems consists mainly of finding the pair of classes $(c_1, c_2)$ for our block $B$.
After this, we need to find the number of ways how the first and second sub-block may look.
These numbers can be precomputed for every combination of class and number of ones in the first sub-block.
After obtaining these numbers of combinations for first and second sub-block, it is trivial to compute the
parameters for sub-routines that decode the sub-blocks and then combine their results to obtain decoded block.
We devised 3 different methods to find the pair $(c_1, c_2)$ for our block $B$. We may precompute how many
blocks there are with class pair equal to $(0, c), (1, c-1),\ldots , (c, 0)$ and name these numbers
$C_{0}\ldots C_{c}$. Example is on Fig.~\ref{table:class_pairs}.
\begin{figure}
	\centerline{
...
as a combination of two 64 bit numbers. 
