...
The graph comparing results for classic bit vectors and hybrid bit vectors can be sobserved in Fig.~\ref{obr:vysledky_sdsl_hybrid}.
The results of our hybrid implementation are not very promising on these data. Except for the access, they do not
look very promising. The reason why the results are better for $\access$ is that hybrid implementation creates
unneccessary memory accesses when anwering $\rank$ and $\select$. 
...
without hybrid encoding. The results may be observed in Fig~\ref{obr:benchmark_sdsl_hybrid_count}.
...
\begin{figure}
	...
	\caption[TODO]{Counting occurrences of patterns in various different texts. Displaying
	the query time per pattern symbol compared to size index. 
	}
