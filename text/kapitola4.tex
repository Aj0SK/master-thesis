...
to the \texttt{rrr\_vector} class. The second change, more impactful on a runtime
of \texttt{rrr\_vector}, changes how we work inside of a superblock. Until now, when
accessing $i$-th bit, we would find the block where the bit is located. To decode the
block and thus obtain this bit, we would need its class and offset. Getting class
is easy as it is located on index $\floor{i/b}$ in array $C$ at an offset that can be easily
computed as $C$ contains elements of the fixed size. Offset is also stored on index
$\floor{i/b}$ in array $O$ but on otherwise unknown offset. To find the offset in array $O$,
we first use the array $P$ that stores offsets to beginnings of superblocks. To continue
further along the superblock, we just need to shift along the blocks preceding the one
we are interested in. As we shown in TODOchapter2, we can do this shifting just using the
information in array $C$ as to shift over $k$-th block, we need to shift by $$\ceil{\log_2 {b\choose C[k]}}$$
bits. With cutoff in place, however, we are not able to linearly scan through the superblock
only using information contained in $C$. This is because now, for some classes, $C$ does
not store the number of ones in the block. Thus when we are shifting across the superblock,
we need to also from time to time access the array $O$, to get the representation of a hybridly
encoded block and count the true number of ones in it. This creates some additional memory accesses
that may slow down the hybrid implementation.
