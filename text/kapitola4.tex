...
In this chapter, we describe the implementation of ideas proposed in previous
chapter and the practical results we obtained from measuring their performance.
We split the chapter into two main parts according to the two main topics discussed
in previous chapter namely our new encoding and decoding method and hybrid encoding.
For both of these ideas, we first describe the important parts and decisions in our
...
that uses the on the fly-decoding and enables us to use block sizes from 3 up to 256. To support
these block sizes, \texttt{SDSL} implements 128-bit and 256-bit integers. On top of this, \texttt{SDSL}
...
elements $O$ that stores offsets of blocks along their class and third array $P$ that stores pointers
...
outperforms binary search most of the time. In practice, we found that sequential search using
the \texttt{SIMD} instructions leads to the best results for smaller block sizes. Possible 
implementations that we tried follow in listings \ref{code:linear}, \ref{code:linearSimd} and
\ref{code:binary}.
...
uint32_t get_left_class(uint8_t k, uint32_t nr)  {
	...
	for (; left_k < left_k_to; ++left_k)  {
		uint32_t curr_index = C[k][left_k+1];
		if (curr_index >= nr)  {
			if (curr_index == nr)
				++left_k;
			break;
...
benchmark. The second type of benchmarks used
