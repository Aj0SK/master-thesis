...
We now turn our attention to the idea of hybrid encoding. In the previous chapter, we proposed two
ideas. Both these were based on a idea not to encode the blocks that are rare and thus save space
on representing the individual classes. On the other hand, they differed in which blocks are not
encoded but just trivially copied. The first solution was treating specially just block with high
number of ones. The second, however, was more focused on balanced sequences and treated specially
blocks that have more balanced number of zeroes and ones. 
