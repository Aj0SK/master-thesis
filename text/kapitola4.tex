...
proposed two variants of hybrid encoding. Both of these were based on a idea not to
encode the blocks that are rare and thus save space on bits used for blocks class. On
the other hand, they differed in which blocks are not encoded but just trivially copied.
The first solution treated differently just block with high number of ones. The second,
was focused on balanced sequences and treated differently blocks that have more balanced
