...
We obtained results presented in this chapter on a machine with 8-core AMD~Ryzen~7~2700X
with 16~MB of cache, running at 3.7~GHz with 16~GB of RAM. The running operating system
was Ubuntu~20.04.3~LTS. We used both \texttt{GCC} version 9.4.0 and \texttt{Clang} version
10.0.0 compilers with the optimizations level most of the time set to \texttt{O3}. Although
there are no special hardware requirements, to obtain the best possible results, our
...
We now turn our attention to the hybrid encoding. In the previous chapter, we proposed two
ideas. Both of these were based on a idea not to encode the blocks that are rare and thus
save space on bits used for blocks class. On the other hand, they differed in which blocks
are not encoded but just trivially copied. The first solution was treating specially just
block with high number of ones. The second, however, was more focused on balanced sequences
and treated differently blocks that have more balanced number of zeroes and ones.

\subsection{Implementation}

Implementing these both encodings required changes to more than only encoding and decoding
...
from range \texttt{cut\_from} up to and including \texttt{cut\_to}. Thus, integer to save
a single class then uses $\ceil{\log_2(c_k+1)}$ bits of space.
