...
offset along this class. To make the comparison fair, we measured decoding time of the bit in the half of
the block for on-the-fly decoding as this decoding can stop once it obtained the queried bit. The results
...
however, harder to use for longer blocks as it ask for support of special instruction set. Version splitting
...
After benchmarking and finding the potential for speeding up the bit vector implementation using
...
version based on uncompressed bit vector.
...
This means that certain implementation may not be the fastest or space optimal but changing it for one
that is faster means increasing space usage and vice versa. These Pareto optimal values can be
...
\begin{figure}
	...
	text. Uncompressed bit vector (sdsl-bv) is included for reference. 
	}
...
This is a parameter that can be used to trade-off between the speed of FM-index and
...
however, note that the trends that can be seen in these results can be observed for every sampling
rate.

We can observe similar speedup as for $\countOp$. The visible difference is slightly
bigger speedup on Source codes data for block length 31. On the DNA data, the speedup is
hitting again 30\%.
...
The results are very strong for DNA, Proteins and Sources datasets but even on English, where
...
$\lg (c_k+1)$ bits of space to represent a single class.
...
The goal of first benchmark was to observe, how the first version of hybrid encoding behaves on randomly generated
sequences with fixed percentage of ones. The reason behind this test is that bit vector implementations based on RRR
do not perform very well on this type of sequences and are often dominated by sparse bit vector implementation.

Thus, we generated a random sequence of bits containing 5\% of ones. and compared our original RRR implementation with hybrid
implementation and also sparse bit vector provided by \texttt{SDSL}. We present the results in Fig.~\ref{obr:vysledky_hybrid_artif}.
We may observe that our implementation was already competitive with sparse array for $\access$ and $\rank$. Hybrid encoding
only amplified these differences. However, on $\select$ where our implementation was dominated by sparse array, these differences
were not completely overcome by hybrid implementation. We can, however, conclude that hybrid version of 63 and 127-bit block
created new viable alternatives, most importantly for $\select$ method.

We tested our second version of hybrid encoding on the previously used datasets \texttt{WT-DNA} and \texttt{WT-WEB}.
The graph comparing results for classic bit vectors and hybrid bit vectors can be observed in Fig.~\ref{obr:vysledky_sdsl_hybrid}.
The results of our hybrid implementation are not very promising on these data, except for the $\access$. The reason
why the results are better for $\access$ is that hybrid implementation creates additional memory accesses when answering
$\rank$ and $\select$ as we described in the previous subsection. 

To test if our hybrid implementation could be used in the practice, we benchmarked FM-index implementation of method $\countOp$,
with hybrid bit vector used inside of the Huffman shaped wavelet tree. The results are shown in Fig~\ref{obr:benchmark_sdsl_hybrid_count}.
...
\begin{figure}
	...
	Every cross symbols hybrid implementation with particular cutoff. The same block size is denoted by the single color.
	Name $\texttt{hybrid-}x\texttt{(}y\texttt{)}$ denotes implementation of block size $x$ with cutoff $y$.
	}
