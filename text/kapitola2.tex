...
every superblock. Then, every superblock is further divided into blocks of length $\ceil{(\log_2 n)/2}$.
...
\begin{enumerate}
    ...
    $\BigO(2^{(\log n)/2}) = \BigO(\sqrt{n})$ blocks of length $\BigO((\log n)/2)$. Number of possible $\rank$
    queries over block is equal to its length. For every element stored in the table we need at most
    $\BigO(\log\log n)$ bits of space so the total amount of space is
    \begin{equation}
...
When answering $select_1(i)$, we first find the location of right superblock. If this
