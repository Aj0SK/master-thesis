...
This chapter is dedicated to the current state of the bit vector implementation.
Bit vector stores binary sequence supporting methods $\access$, $\rank$ and
$\select$. As we have shown in section~\ref{section:WaweletTree}, wavelet tree
can be used to build a vector for the general alphabet using the bit vector
implementation. This is, why it is of the utmost importance to dedicate time
to build fast and efficient bit vector implementation. Let us start with the
...
and $\rank_1(i)$. It is easy to observe that in the binary sequence following equation
holds:
                    $$\rank_1(i) = i - \rank_0(i)$$
so it is common to provide only the implementation of $\rank_1(i)$. There are two
straightforward solutions we can begin with to support $\rank$ on a bit vector
$B$. The first is not to precompute any additional information. Every time we want
...
bit. This enables us to answer $\rank$ in constant time -- a single table lookup.
...
pre-computed $\rank$ values, but not for every bit. At first, we choose a constant
$k$ and split bit vector into non-overlaping subsequences of length $k$. We shall
call these subsequences \textit{superblocks}. We then precompute $\rank$ only for
the beginning of every superblock. This enables answering $\rank$ in time $\BigO(k)$
as we need to only count bits from the start of the superblock that the queried bit
is located in. The stored information use $\BigO((n/k)\log n)$ bits of memory as
there is $n/k$ superblocks and every rank can be of size at most $n$.
...
beginning of superblock. Every superblock we further divide into blocks of size $(\log n)/2$.
We precompute the $\rank$ for all of these smaller blocks but to save space we only precompute
the $\rank$ from the beginning of the corresponding superblock. Now, when queried for $\rank_1$
of some position, we know in which superblock this number is located, so we know what is the
numberof ones up to the beginning of the superblock where this position is located. We also
...
constant time $\rank$ so we need to also precompute every possible $\rank$ query inside of the
block. Thanks to block being short there are not that many possible ways what the block can be,
and what is the position in this block we want to compute $\rank$ for. The space overhead that
this solution adds consists of three parts. The first part is precomputed $\rank$ for every
superblock. Number of superblocks is $\BigO(n/\log^2 n)$ and we need $\BigO(\log n)$
bits to store the every single $\rank$ value so the total amount is
\begin{equation}
...
only from the beginning of superblock so this number is of size at most $\BigO(\log n)$. Thus
we need at most $\BigO(\log\log n)$ of bits to store it. The total amount of space for the
...
possible $\rank$ query over it the result of the $\rank$ query. The number of different
ways for what the block can be is $2^{(\log n)/2} = \sqrt{n}$. Number of possible $\rank$
queries over it is $\BigO(\log n)$ so this makes the table of the size 
\begin{equation}
...
\ref{eq:rank_space_3}. As every one of these is using sublinear number of bits we can see
that the solution uses just sublinear space.
...
and $select_1$. Although there is not a way how to convert one to another just like with
$rank$, we shall be interested mainly in $select_1$ version as the other can be easily
...
of $\select_c(i)$ if we have a efficient implementation of $\rank$. This can be nicely combined
