...
bits of memory as there are $\ceil{n/k}$ superblocks and every rank can be of size at most $n$.
...
requires less work to be done inside of the superblock.
...
% TODO: in theory, for k ~ log n we already get linear space and O(1) time if we assume a strong-enough model (RAM with log n bit registers with popcount)
...
\begin{figure}
	...
	\end{tabular}
    % TODO: oddelit tie tabulky medzerou, netusim ako to urobit jednoducho
    % nefunguje newline, medskip, bigskip, vspace
    \begin{tabular}{c|c|c|c|c|}
        \cline{2-5}
...
Then there are superblocks shorter than $t_1^2$ called
