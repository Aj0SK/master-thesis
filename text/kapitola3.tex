...
blocks along the class $c$ are primarily sorted by class pair $(c_1, c_2)$ so blocks
with the same class pair form a consecutive sequence. For every class $c$, we precompute
numbers $Z_{max(c-b_2, 0)},\ldots , Z_{max(c-b_1, 0)}$ where $Z_i$ is the smallest offset
of block having class pair $(i, c-i)$.

Assume that our block has the class pair equal to $(c_1, c_2)$. The second step is to
is to find $o_1$ and $o_2$. There are more blocks that have the same class pair
