...
along the same class and we no longer use the lexicographical ordering. Note that we
...
with sub-blocks of size $b_1$ and $b_2$. We first count the number of ones to
obtain class $c=\#_1(B)$. Then we compute the offset by counting the number of
preceding blocks. These can be divided into 3 categories:
\begin{enumerate}
...
$$\sum_{i=0}^{c_1-1} {b_1\choose i} {b_2\choose c-i}$$. For fixed $i$, ${b_1\choose i}$ denotes the
number of sub-blocks with $i$ ones and ${b_2\choose c-i}$ denotes the number of sub-blocks with the rest
$c-i$ ones.

The number of blocks in group~\ref{chapter3:encoding:2} is equal to $$o_1\times {b_2\choose c_2}.$$
These are all the blocks with first sub-block smaller than $B_1$ and any second sub-block with
$c_2$ ones.

The number of blocks in group~\ref{chapter3:encoding:3} is equal to $o_2$. This is number of blocks that
have $B_1$ as the first sub-block and smaller second sub-block $B_2$.

As we mentioned, the offset of block $B$ is given by the sum of sizes of these 3 groups. However,
to obtain these, we need to get $c_1, c_2, o_1$ and $o_2$. These numbers can be obtained by recursively
encoding the sub-blocks $B_1$ and $B_2$.
...
\end{figure}
We may observe, that for some choices of cutoff, we may expect to save rougly 10--17\%
