...
Then, we show how we can exploit the assumptions about the input sequence to build
an implementation better tailored for the underlying bit sequence.
...
The main idea behind our solution is to use a divide-and-conquer approach.
...
the same class. We want to note that we also use the number of ones to identify class
...
to their bit concatenation $B_1\cdot B_2$. Primarily, the block is compared according to its
\textit{class pair} $(c_1, c_2)$ where $c_1$ and $c_2$ are the classes of the smaller sub-blocks
$B_1$ and $B_2$. Only then, the tie is broken by order of $B_1$ and then by $B_2$ along their respective
classes. Note that the original lexicographical ordering can be rephrased in the context of sub-blocks as
sorting at first by $B_1$ and then by $B_2$. The example of the new ordering for particular block
length and class is shown in Fig.~\ref{obr:lexicographicalVsUs}.
...
When convenient, in this situation we refer to block $B$ as being smaller than $B'$. 
We write $B\prec_{\Lex} B'$ if $B$ preceeds $B'$ in lexicographical
ordering. Using this new notation, our new proposed ordering $P$ can be formalized
as
\begin{align*}
    B\prec_P B' \iff
    &[\#_1(B_1) < \#_1(B_1')] \\
...
\end{align*}
where $B=B_1\cdot B_2$, $B'=B_1'\cdot B_2'$ and $X$, $Y$ are some orderings.

We now demonstrate how encoding and decoding works with our proposed ordering.
...
The number of blocks in group~\ref{chapter3:encoding:2} is equal to $o_1\times {b_2\choose c_2}$
as $o_i$ denotes the number of sub-blocks smaller than the $B_1$ in its class and ${b_2\choose c_2}$
denotes number of possible combination for second sub-block. The number of blocks in
group~\ref{chapter3:encoding:3} is equal to the number of blocks smaller than $B_2$.
This is identical to the offset of second sub-block -- $o_2$. Note, that to obtain the offset of
$B$ we need the encoded representation of the sub-blocks $B_1$ and $B_2$. These are two pairs of numbers
...
second sub-blocks) and will repeat 3 times (number of possible first sub-blocks). Along
these ordering of 9 blocks we are interested in 7th so we know that the block,
we are looking for contains a
...
sorted by class pair $(c_1, c_2)$. Let us denote the number of blocks with class pair $(c_1, c_2)$
$C_{c_1}$. If we know the exact values of the number $C_0, C_1, \ldots , C_c$, we can search
...
\paragraph{Hybrid encoding for balanced sequences}

Although, this version of hybrid encoding may work well for sequences with low number of ones.
We would like to use this idea as a viable solution for sequences that contain very similar
number of zeroes and ones but still exhibit behavior that some block classes are underrepresented.
...
ones are more frequent than blocks with balanced count of zeroes and ones.

Let us say, that we want to only store $r$ possible classes. To make some space saving possible,
it must hold that $$\ceil{\log_2(r)} < \ceil{\log_2(b+1)}.$$ We shall create a range of classes
given by closed interval $[\mathit{cut\_from}, \mathit{cut\_to}]$. If the class is outside of
its range, it would not be altered. All the values inside of this interval are mapped to one
number. This could be without loss of generality the lowest value $\mathit{cut\_from}$.
Example of this mapping can be observed in Fig.~\ref{obr:hybrid_encoding_valley}. 
...
\begin{figure}
	...
	\textbf{Hybrid mapping} & 0 & 1 & 3 & 3 & 3 & 3 & 3 & 7 \\ \cline{2-9} 
	\end{tabular}
	}
	\caption[TODO]{Example for $b=8, \mathit{cut\_from}=2, \mathit{cut\_to} = 6$}
	\label{obr:hybrid_encoding_valley}
\end{figure}

In this example, the number of different possible class values $r$, is equal to 4.
However, using this representation, we would not save any space as the maximal value
of class can be still of size 7, requesting the same number of bits to store as
in initial scenario. It is easy to observe however, that these value can be mapped
somehow into the range $[0, 3]$, thus only consuming two bits per class value.