...
We write $B\prec_{\Lex} B'$ if $B$ preceeds $B'$ in lexicographical ordering.
...
It is easy to observe, that the main part of space-saving aspect of RRR is hiddden in
...
class and $\ceil{\log_2{b\choose c}}$ bits to store the offset. We may observe in
Fig.~\ref{obr:rrrSpaceSavings}, how for different block sizes the space saved per
block depends on the blocks class. At the same time, RRR saves most of the memory
on very sparse or very dense blocks. We would like to exploit situations, when
the number of these blocks is very high. Let us consider a randomly generated bit
sequence $B$ of length $n$ containing $p\%$ of ones. Let us analyze what is the
probability distribution of the block classes in a sequence containing this fixed
percentage of ones distributed randomly. 
...
If we consider a block size $b$ then the random variable $X$ denoting number of
...
This gaved us opportunity to a encoding method we named \textit{hybrid encoding}.
...
is stored using full $b$ bits instead of $\ceil{\log_2{b\choose c_i}}$ bits.
...
Although, the previous version of hybrid encoding may work well for sequences with low number of ones.
We would like to use this idea as a viable solution for sequences that contain similar frequency
of zeroes and ones but still exhibit behaviour that some block classes are underrepresented.
...
ones are more frequent than blocks with balanced counts of zeroes and ones.

Let us say, that we want to only store $r$ possible classes. To make some space saving possible,
it must hold that $$\ceil{\log_2(r)} < \ceil{\log_2(b+1)}.$$ We create a range of classes
given by closed interval $[\mathit{cut\_from}, \mathit{cut\_to}]$. If the class is outside of
its range, its representation would not be altered. On the other hand, all the values inside of
this interval are mapped to one particular class number. This could be without loss of generality
the lowest value $\mathit{cut\_from}$. An example of this idea can be observed in
Fig.~\ref{obr:hybrid_encoding_valley}. 
...
of class can be still at most 7. This would consume the same number of bits to store as
in original representation. It is easy to observe, however, that these values can be
mapped into the range $[0, 3]$, thus only consuming two bits per class value.
