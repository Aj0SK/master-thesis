...
the first sub-block only from the offset of the block, we can look into the table in
Fig.~\ref{obr:lexicographicalVsUs} and see that there are 3 blocks with zero ones in
the first sub-block. As the offset of our block is ten, we need to look for this block
between blocks with more than 0 ones in the first sub-block. There are 9 blocks with one
1 in the first sub-block. As $3+9$ is bigger than 10, we discovered that the number of
ones in the first block is 1 and it follows that the number of ones in the second sub-block
is also 1 as we know the total number of ones in the block is 2. There are, however, 9 blocks
with this exact same property, the smallest one being {\tt 001 001} and the largest one
{\tt 100 100} as can be seen in Fig.~\ref{obr:lexicographicalVsUs} (offsets 3--11). We have
successfully narrowed our search into these 9 blocks and now we can focus on identifying the
offset of the first and second sub-block.

There are ${3 \choose 1} = 3$ possibilities (namely {\tt 001}, {\tt 010}, {\tt 100})
how the first and also the second sub-block may look. Blocks are sorted according to the first
and then the second sub-block. The cycle through the sub-blocks will be of length 3 (number of
possible second sub-blocks) and will repeat 3 times (number of possible first sub-blocks). To
find the offset of the first and the second sub-block, we need to find out on which first and second
...
the first sub-block {\tt 100} and pair $(1, 7\bmod 3)$, which gives us the second sub-block.

Let us propose a general decoding scheme for block $B$ of length $b$ combined from the
sub-blocks of sizes $b_1$ and $b_2$. All we have at the beginning of decoding is the
...
blocks along the class $c$ are primarily sorted by class pair $(c_1, c_2)$ and thus
blocks can be categorized by their class pair into one of the groups $G_0, G_1,\ldots G_c$
with block having class pair $(i, c-i)$ being categorized into $G_i$. The important property
is that for any numbers $i, j$ such that $0\leq i<j\leq c$, blocks from group $G_i$ have all
smaller offsets than blocks from group $G_j$. It is possible to precompute how many blocks
are contained in some particular group $G_i$ thus it is also possible to precompute for every
group $G_i$ a number $Z_i$. This is the smallest offset of some block in group $G_i$.
Searching for class pair of block $B$ is then easy. We have its offset $o$ and for its class
$c$ we look at the precomputed numbers $Z_0, Z_1,\ldots Z_c$ and find the right bucket.
For $o$ it is the last $Z_i$ such that $Z_i\leq o$.

Assume that our block has the class pair equal to $(c_1, c_2)$. The second step is to
is to find $o_1$ and $o_2$. There are $|G_{c_1}|$ blocks that have the same class pair
